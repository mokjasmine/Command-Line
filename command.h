// UCLA CS 111 Lab 1 command interface

#include <stdbool.h>

typedef struct command *command_t;
typedef struct command_stream *command_stream_t;

typedef struct Stack {
    struct commandNode *top;
    int size;
}Stack;

typedef struct command_stream{
  struct commandNode *head;
  struct commandNode *tail;
  struct commandNode *cursor;
}command_stream;

typedef struct GraphNode
{
	command_t command;
	struct GraphNode** before;
	pid_t pid;
	struct GraphNode* next;
}GraphNode;

typedef struct commandNode{
  struct command *command;
  struct commandNode *next;
  struct GraphNode *graphnode;
}commandNode;

/*
typedef struct listItem{
	struct GraphNode* node;
	struct listItem *next;
}listItem;
*/

typedef struct dependencyGraph
{
	struct GraphNode* no_dependencies;
	struct GraphNode* dependencies;
}dependencyGraph;

//variables
extern int numChars;
extern struct command* commandArray;
extern char** wordArray;
extern char*** readListArray;
extern char*** writeListArray;
extern int readListArray_size;
extern int writeListArray_size;

/* Create a command stream from GETBYTE and ARG.  A reader of
   the command stream will invoke GETBYTE (ARG) to get the next byte.
   GETBYTE will return the next input byte, or a negative number
   (setting errno) on failure.  */
command_stream_t make_command_stream (int (*getbyte) (void *), void *arg);

/* Read a command from STREAM; return it, or NULL on EOF.  If there is
   an error, report the error and exit instead of returning.  */
command_t read_command_stream (command_stream_t stream);

/* Print a command to stdout, for debugging.  */
void print_command (command_t);

/* Execute a command.  Use "time travel" if the flag is set.  */
void execute_command (command_t, bool);

/* Return the exit status of a command, which must have previously
   been executed.  Wait for the command, if it is not already finished.  */
int command_status (command_t);	

void push(Stack *S, command_t newCommand);
struct commandNode* top(Stack *S);
void pop(Stack *S);



